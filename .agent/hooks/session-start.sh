#!/bin/bash
# Checks for the code-graph knowledge graph and outputs
...
if [ -f "$DB_PATH" ]; then
    cat <<'INSTRUCTIONS'
[code-graph] Knowledge graph is available.

When answering questions about this codebase, prefer using the code-graph MCP tools before scanning files manually:
...
INSTRUCTIONS
    ...
    echo "[code-graph] No knowledge graph found. Run /code-graph:build-graph to parse this codebase and enable graph-powered queries."
fi
